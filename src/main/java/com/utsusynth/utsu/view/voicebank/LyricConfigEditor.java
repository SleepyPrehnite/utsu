...
public class LyricConfigEditor {
	...
	public GridPane createConfigEditor(LyricConfigData config) {
		...
		double preutterBarX = Math.min(Math.max((offsetLength + config.preutterProperty().get()) * scaleX, 0), totalX);
		double overlapBarX = Math.min(Math.max((offsetLength + config.overlapProperty().get()) * scaleX, 0), totalX);
		...
		controlBars.getChildren().setAll(offsetBar, overlapBar, preutterBar, consonantBar, cutoffBar);
		...
		offsetBar.setOnMouseDragged(event -> {
			changed = true;
			double newX = Math.min(Math.max(event.getX(), 0), cutoffBar.getStartX() - consonantPane.getPrefWidth());
		...
		consonantBar.setOnMouseDragged(event -> {
			changed = true;
			double newX = Math.min(Math.max(event.getX(), offsetBar.getStartX()), cutoffBar.getStartX());
	...
	public void playSound() {
		...
		mediaPlayer.setOnReady(() -> {
			...
			double offsetLengthMs = Math.max(Math.min(configData.get().offsetProperty().get(), lengthMs), 0);
	...
	private Line createControlBar(LyricConfigData config, double xPos, double totalX, String style) {
	...
	public ImageView createSpectrogramChart(LyricConfigData config) {
		File pathToWav = config.getPathToFile();		
		Optional<WavData> wavData = soundFileReader.loadWavData(pathToWav);
		Wave configWave = new Wave(pathToWav.getAbsolutePath());
		Spectrogram configSpectrum = configWave.getSpectrogram();
		//double waveLength = configWave.length();
		ImageView view = new ImageView();
		//view.setImage(null);
		//double nx = 0;
		double[][] spectrumData = configSpectrum.getAbsoluteSpectrogramData();
		double[][] plotData = configSpectrum.getNormalizedSpectrogramData();
		int nX = spectrumData.length;
		int nY = spectrumData[0].length;
		 //plot image
		double width = (int)(wavData.get().getLengthMs() * scaleX);
        ...
        view.setImage(SwingFXUtils.toFXImage(theImage, null));
        view.setFitWidth(width);
        view.setFitHeight(nY);
		return view;
	}
